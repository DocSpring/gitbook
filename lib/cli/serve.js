    ...
    function generateBook() {
        return Parse.parseBook(book)
            .then(function (resultBook) {
                // Set the path prefix after parsing book config.
                // Gets added to the X-Current-Location header, so that
                // we can proxy the dev server on a different path.
                // e.g. localhost:3000/docs/
                server.path = resultBook.config.getValue('serverPath');
